...
echo       whisper\modelos\                modelo tiny
...
echo --- [4/7] Modelo do whisper (tiny)
set "WBIN=%WMOD%\ggml-tiny.bin"
if exist "%WBIN%" (
  echo      [ok] ja baixado.
) else (
  echo      baixando; e grande, pode demorar bastante.
  curl -L --fail --progress-bar -o "%WBIN%" "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-tiny.bin" || (
...
> "%RAIZ%\LEIA-ME.txt" (
  ...
  echo whisper\modelos    - ggml-tiny.bin
...
if %WMB% LSS 60 (
  echo      [ERRO] veio so %WMB% MB; o modelo tiny tem uns 60 MB ou mais.
