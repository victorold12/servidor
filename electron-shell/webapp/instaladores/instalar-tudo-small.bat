...
echo       whisper\modelos\                modelo small
...
echo --- [4/7] Modelo do whisper (small)
set "WBIN=%WMOD%\ggml-small.bin"
if exist "%WBIN%" (
  echo      [ok] ja baixado.
) else (
  echo      baixando; e grande, pode demorar bastante.
  curl -L --fail --progress-bar -o "%WBIN%" "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-small.bin" || (
...
> "%RAIZ%\LEIA-ME.txt" (
  ...
  echo whisper\modelos    - ggml-small.bin
...
if %WMB% LSS 400 (
  echo      [ERRO] veio so %WMB% MB; o modelo small tem uns 400 MB ou mais.
