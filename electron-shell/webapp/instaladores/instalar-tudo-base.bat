...
echo       whisper\modelos\                modelo base
...
echo --- [4/7] Modelo do whisper (base)
set "WBIN=%WMOD%\ggml-base.bin"
if exist "%WBIN%" (
  echo      [ok] ja baixado.
) else (
  echo      baixando; e grande, pode demorar bastante.
  curl -L --fail --progress-bar -o "%WBIN%" "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-base.bin" || (
...
> "%RAIZ%\LEIA-ME.txt" (
  ...
  echo whisper\modelos    - ggml-base.bin
...
if %WMB% LSS 120 (
  echo      [ERRO] veio so %WMB% MB; o modelo base tem uns 120 MB ou mais.
