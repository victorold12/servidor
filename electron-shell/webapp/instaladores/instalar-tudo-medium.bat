...
echo       whisper\modelos\                modelo medium
...
echo --- [4/7] Modelo do whisper (medium)
set "WBIN=%WMOD%\ggml-medium.bin"
if exist "%WBIN%" (
  echo      [ok] ja baixado.
) else (
  echo      baixando; e grande, pode demorar bastante.
  curl -L --fail --progress-bar -o "%WBIN%" "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-medium.bin" || (
...
> "%RAIZ%\LEIA-ME.txt" (
  ...
  echo whisper\modelos    - ggml-medium.bin
...
if %WMB% LSS 1200 (
  echo      [ERRO] veio so %WMB% MB; o modelo medium tem uns 1200 MB ou mais.
